...
def audit_ingestion(store: VectorStore, image_dir: Path) -> dict:
    ...
    for cat in CATEGORIES:
            ...
            rapport["categories"].append(
                {
                    ...
                    "nb_fragments": 0,
                    "confiance": "FAIBLE",
                    "resume": "Aucun fragment trouvé.",
                    "points_flous": ["Section non reconnue."],
                }
            )
            continue
        ...
        prompt = (
            f"Évalue la qualité de l'extraction pour {cat} en JSON : "
            ...
            if data.get("confiance") == "FAIBLE":
                for point in data.get("points_flous", []):
                    rapport["questions_humain"].append(
                        {"categorie": cat, "question": f"[{cat}] {point}"}
                    )
    ...
    if image_dir.exists():
        ...
        for img_path in images[:5]:
            ...
            prompt = (
                "Analyse cette image RFP en JSON : "
                '{"page": "'
                + page_no
                + '", "lisibilite": "BONNE/MOYENNE/MAUVAISE", "elements_identifies": [], "questions_pour_humain": []}'
            )
            try:
                ...
                if data.get("lisibilite") == "MAUVAISE":
                    for q in data.get("questions_pour_humain", []):
                        rapport["questions_humain"].append(
                            {"categorie": f"VISUEL P.{page_no}", "question": q}
                        )
            except Exception as e:
    ...
    for cat in ["TECHNIQUE", "SECURITE"]:
        ...
        for frag in fragments[:3]:
            try:
                ...
                r1 = ollama.generate(
                    model=TEXT_MODEL,
                    prompt=f"Extrais exigence en JSON : {frag['text'][:800]}",
                    format="json",
                )
                ...
                # 2. Match catalogue
                know_how = catalog_store.search_hybrid(
                    query=req.get("exigence", ""), n_results=3
                )
                context = (
                    "\n".join([k["text"][:400] for k in know_how])
                    if know_how
                    else "AUCUN SAVOIR-FAIRE."
                )
                ...
                r2 = ollama.generate(
                    model=TEXT_MODEL,
                    format="json",
                    prompt=f"EXIGENCE : {req.get('exigence')}\nSAVOIR-FAIRE : {context}\n"
...
def generate_markdown_report(ingestion: dict, gap: list, rfp_name: str) -> str:
    ...
    for cat in ingestion["categories"]:
        emoji = (
            "✅"
            if cat["confiance"] == "ÉLEVÉE"
            else "⚠️"
            if cat["confiance"] == "MOYENNE"
            else "❌"
        )
