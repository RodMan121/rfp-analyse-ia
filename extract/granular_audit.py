...
def save_fsm_registry(fsm_requirements):
    """Sauvegarde les objets FSM pour être lus par le Composer."""
    output_path = Path("data/fsm_registry.json")
    serializable = []
    for r in fsm_requirements:
        d = asdict(r)
        d['state'] = r.state.value # Conversion Enum
        serializable.append(d)
        
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(serializable, f, indent=2, ensure_ascii=False)
    logger.success(f"🗄️ Registre FSM sauvegardé : {output_path}")
...
def generate_markdown_report(results: list, category: str):
    ...
    for r in results:
        ...
        if f.state != RequirementState.AUDITED:
            ...
            report += f"| {f.uid} | **{f.state.value}** | {r['original']} | `{fuzzy}` |\n"
...
def display_summary_table(results: list):
    ...
    for r in results:
        ...
        table.add_row(f.uid, f"[{state_style}]{f.state.value}[/]", f"{f.ambiguity_score}/100")
    console.print(table)
