...
class RFPAgent:
    ...
    def _route_request(self, question: str) -> str:
        ...
        try:
            response = ollama.generate(
                model=self.text_model, prompt=router_prompt, options={"num_predict": 5}
            )
            ...
            return (
                "VISION" if any(kw in question.lower() for kw in keywords) else "TEXT"
            )
    ...
    def _summarize_history(self):
        ...
        history_text = "\n".join(
            [f"{m['role']}: {m['content']}" for m in self.history[:-2]]
        )
        ...
        try:
            ...
            self.history = [
                {
                    "role": "system",
                    "content": f"Résumé: {response.get('response', '')}",
                },
                *self.history[-2:],
            ]
        except Exception:
    ...
    def _ask_text(self, question: str, stream: bool = True) -> str:
        ...
        if not initial_results:
            return "⚠️ Aucun fragment trouvé."

        best_results = self.reranker.rerank(
            query=question, documents=initial_results, top_n=5
        )
        ...
        for r in best_results:
            source_info = (
                f"SOURCE: {r['metadata']['breadcrumbs']} (P.{r['metadata']['page']})"
            )
...
RÉPONSE :"""
        ...
        if stream:
            response_gen = ollama.generate(
                model=self.text_model, prompt=prompt, stream=True
            )
    ...
    def _ask_vision(self, question: str) -> str:
        ...
        if not results:
            return "⚠️ Impossible d'identifier la page."
        ...
        response = ollama.generate(
            model=self.vision_model, prompt=prompt, images=[str(image_path)]
        )
        return (
            f"**(Analyse Visuelle - Page {page_no})**\n\n{response.get('response', '')}"
        )
